...
- **ABI signature id** (32-bit hash of the event name + input types) — how the decoder recognizes a body. Built at **compile time**: `crates/infrastructure/src/decoder.rs` embeds the ABIs via `include_str!` (`DEX_ABIS`: RootOracle, Oracle, OracleEventList, PMP, OrderBook, RootPN, PrivateNote, Nullifier; `INFERENCE_ABIS`: InferenceOrderBook, TokenContract, RootModel, SuperRoot). The last two are the registry side: nothing projects them, and they are loaded so their events decode rather than landing with `event_type` NULL — an undecodable row is never projected, so `processed_at` stays NULL and `prune_raw_events` (processed rows only) can never reclaim it. RootModel is also load-bearing for routing: it resolves the `ContractDeployed` id collision with `TokenContract` via `add_route` on 703/732. Two ABIs ship in the repo and are NOT loaded — `ModelRegistry` (which declares no events at all, so loading it would change nothing) and `Multisig`. Changing an `.abi.json` requires rebuild + redeploy.
...
- **Forgetting the decoder count pins**: any event add/remove breaks the two `decoder.rs` tests asserting `== 83` — update both literals and the arithmetic comments next to them (the two calls return the same value, so they fail and pass together).
