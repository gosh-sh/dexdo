...
//! contract sources it claims to mirror.
//!
...
//!
//! - an id **missing** from it means one of our own events is dropped at ingest,
//!   never reaches `raw_events`, and cannot be recovered by reprojection — the
//!   gateway's event window is finite;
//! - an id **stale** in it means foreign traffic keeps flowing into `raw_events`
//!   under an id we no longer emit.
//!
...
/// The EVENT_ID of every event the contracts actually route to an external
/// address, read off the `makeAddrExtern(<CONST>, bitCntAddress)` call sites.
fn emitted_event_ids() -> BTreeSet<u32> {
...
fn scoped_event_ids_match_the_contracts() {
    ...
    assert!(
        missing.is_empty(),
        "contracts emit event ids the indexer does not capture: {missing:?}. Add them to \
         config::SCOPED_EVENT_IDS — until then those events are dropped at ingest and are NOT \
         recoverable by reprojection."
    );
    assert!(
        stale.is_empty(),
        "config::SCOPED_EVENT_IDS carries ids no contract emits: {stale:?}. Remove them — each \
         one admits foreign traffic under an id we do not use."
    );
}
