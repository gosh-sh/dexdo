...
| `reconcile_attempts` | `integer` default `0` | Diagnostic counter. |
| `last_swept_at` | `timestamptz` (nullable) | Stamped by the reconciler when a sweep cycle completes (all `OPEN` orders in the book have been checked). NULL until the first completed sweep. Used with `inference_markets_sweep_idx` to drive Queue B sweep scheduling. |
