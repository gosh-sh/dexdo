...
- **`InferenceOrderBook.InferenceOrderPlaced`** (live) records `price_per_tick` when the placement is a SELL offer carrying a `tokenContract`: that placement is what registers the deal, and its `price` is the per-tick ask the deal's constructor was given. NOT taken from `InferenceFilled.clearingPrice`, which is the price the match struck — equal to the ask only when the seller was the maker.
- **`InferenceOrderBook.InferenceFilled`** (live) creates or enriches the deal cross-link — `orderbook_address`, `seller_note`, `buyer_note` — and records `deposit`. The deposit is REPRODUCED, not read: the book derives `debit = ticks * (p + p * PLATFORM_FEE_BPS / BPS_DENOMINATOR) + bond` inside `_match` and emits none of it, while every input is emitted. `bond` is `2 * clearingPrice` for a subscription and zero otherwise, and the subscription bit is read off the BUY leg — on the orphan path, where no leg is present, `deposit` is left NULL rather than computed without a bond that may be owed. The two rate constants are pinned against the Solidity source by `dodex-contracts/tests/order_book_fee_constants.rs`. What it records is what the book SENT: on the narrow path where `fundFromOrderBook` refuses the fill and refunds in full, the deal never stores a deposit and this column names an escrow that came straight back.
