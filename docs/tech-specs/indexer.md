...
3. Try to decode the message body against the ABI bundle (`crates/infrastructure/src/decoder.rs`). Every event currently resolves by its **unique event id**: as of v4.0.10 the inference book renamed its events with an `Inference` prefix (e.g. `InferenceOrderCancelled`), so there is no longer an id collision with `OrderBook.OrderCancelled` and no events share an id. The decoder still does **not** assume ids are globally unique — its id index tolerates collisions (one id may map to several `(contract, event)` entries) and a small `dst` route table (keyed on the `makeAddrExtern(EVENT_ID, 256)` address in the message header) can disambiguate any future colliding pair; `OrderBook.OrderCancelled` is pinned to its emit dst defensively. A colliding id with no route is left undecoded as an `AmbiguousCollision` (counted/alertable) rather than mis-attributed. On success, store the decoded JSON payload alongside `event_type`.
...
The projection loop applies the `inference_orphan_cutoff_ms` window as a dead-letter for inference events that have waited beyond the cutoff without their parent arriving. Specifically: an `InferenceOrderBook.InferenceFilled` or `InferenceOrderBook.InferenceOrderCancelled` event whose `raw_events.created_at` (wall-clock ingest time) is older than `inference_orphan_cutoff_ms` and whose parent `InferenceOrderPlaced` row is absent is dropped (marked `Applied` with a counter increment) rather than deferred forever. The cutoff is keyed on ingest time — a row with an old `created_at_chain` but recent `created_at` (e.g. a late-arriving event) is not dropped.

> 🚧 **Deferred — model-id source.** The order book carries only `model_hash`; the human `producer--model--version` is not on the book. Since v4.0.10 the `InferenceOrderBookDeployed` event also carries `modelName`, and the book exposes a `getModelName` getter — either could backfill the name — but this is not yet wired (the `ManifestMetadata` contract that earlier held the manifest was removed upstream). Filling `model_ref` / `producer` / `model_name` / `version` requires a registry walk (`SuperRoot` → `RootModel`) that is not yet implemented. These columns stay NULL; the API exposes the book by `model_hash` only until the linkage is added.
