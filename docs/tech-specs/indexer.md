...
| `OrderBook.OrderCancelled` | `status = 'CANCELLED'`, preserves `amount_remaining` as the unfilled cancelled remainder, monotonic `last_chain_order` update. Advances `chain_updated_at` via `greatest`. |
