...
| `OrderBook.OrderCancelled` | Preserves `amount_remaining` as the unfilled cancelled remainder and flips `status` to `CANCELLED` unless the row is already `FILLED`; the FILLED guard prevents a late cancel event from demoting a fully filled row. Monotonic `last_chain_order` update. Advances `chain_updated_at` via `greatest`. |
