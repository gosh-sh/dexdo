...
- `deadline: null` means the indexer knows of no deadline, and it conflates two chain states the client cannot separate. `placeBuyOrder` accepts `deadline == 0` ("no deadline"), so a GTC bid legitimately has none; a resting SELL, by contrast, always carries a non-zero deadline — the contract rejects a zero-deadline offer as malformed — so a null there is missing data, not a GTC ask; a subscription is not a third case — the contract fixes its term at one month and the order carries no duration of its own, so a subscription placed with deadline 0 rests with 0 and its `null` is the GTC one, permanently. Do not let `null` read as "no deadline". Likewise `createdAt` / `updatedAt` may be `null`.
