...
#
# Examples:
#   After tester: scripts/validate-implementation.sh "prompts/tester/func-...-tests.md" ".claude/roles/reviewer-review-tester.md" "test/"
#   After coder:  scripts/validate-implementation.sh "prompts/coder/func-...-code.md" ".claude/roles/reviewer-review-coder.md" ":!test/"
#
# Exit codes:
# 0 = APPROVED
# 1 = CONCERNS
...
- Active plan: docs/worker/plan.md (if a Step is in progress, also check the implementation against that Step's scope, forbidden residual patterns, naming invariants, and acceptance criteria)
...
2. From the prompt and diff, extract changed/removed/renamed identifiers, DB columns, config keys, SQL table/column names, enum/state names, handlers, and test helpers.
3. For each extracted term that could have residual call sites, run targeted rg searches across the relevant roots, not only changed files. Typical roots: apps/worker/src test db docs config.example.json package.json CHANGELOG.md.
4. For model-change or removal steps, residual search is mandatory. Check every hit and classify it as expected history/docs/migration/test coverage or a real leftover.
...
- for schema/model removal prompts, search existing tests for old fixtures and old expectations; if unlisted tests will fail or preserve forbidden behavior, return CONCERNS
...
- for DB migrations, check forward migration, rollback migration, data preservation, indexes/constraints, foreign-key implications, and tests/fixtures that run all migrations
- for config/API/filter changes, check both construction and all call sites; absence of a key is different from key-with-undefined when the prompt says so
