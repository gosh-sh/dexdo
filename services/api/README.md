...
- Write API implementation (order placement / cancellation / batching): [docs/tech-specs/write-api.md](../../docs/tech-specs/write-api.md). Covers `POST /api/v1/order`, `DELETE /api/v1/order`, and `POST /api/v1/batchOrders` today; `DELETE /api/v1/batchOrders` and `DELETE /api/v1/openOrders` are stub sections inside that doc.
...
- `chain`: Acki Nacki gateway endpoint (`gateway_endpoint`) the trading
  ...
  `cancel_order_timeout_ms`, and `place_batch_timeout_ms` bounding the
