...
- **`RootModel.TokenContractRegistered` (external id 702) is decoded instead of being stored undecodable.** Its ABI was not loaded, so these rows landed in `raw_events` with `event_type` NULL. The projection loop only picks rows that carry an `event_type`, so `processed_at` stayed NULL forever and `prune_raw_events` — which deletes processed rows only — could never reclaim them. New rows decode and project as a no-op.
