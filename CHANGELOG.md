...
- **`inference_deals.price_per_tick` and `deposit` fill again**, from the two book events that survived the scope change. `price_per_tick` comes from the SELL placement that registers the deal (`InferenceOrderPlaced`, whose `price` is the per-tick ask the deal's constructor was given) — not from `InferenceFilled.clearingPrice`, which is the price the match struck. `deposit` is REPRODUCED from `InferenceFilled`: the book derives `ticks * (price + platform fee) + bond` inside its matcher and emits none of it while emitting every input. For a subscription the bond is `2 * clearingPrice`; the bit that says so is read off the BUY leg, so on an orphaned fill (no leg projected) the deposit is left NULL rather than computed short. `deposit` records what the book SENT — on the narrow path where the deal refuses the fill and refunds in full, it names an escrow that came straight back.
